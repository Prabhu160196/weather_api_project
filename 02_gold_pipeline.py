...
# MAGIC %md
# MAGIC What is the latest weather reading for each city right now

# COMMAND ----------

# DBTITLE 1,What is the latest weather reading for each city right now
...
# DBTITLE 1,What are the hourly averages for each city over the last 24 hours
from pyspark.sql import functions as F
...
filtered_df.write \
    ...
    .saveAsTable("weather_project.gold.gold_hourly_summary")
# View result
display(filtered_df)
...
# DBTITLE 1,What were the daily high, low, and peak conditions for each city
from pyspark.sql import functions as F
...
gold_daily_extremes = (
    ...
    .agg(
        ...
        F.sum("rain_fall").alias("total_rainfall"),
...
# DBTITLE 1,Which cities are currently in or recently entered severe weather conditions
from pyspark.sql import functions as F
from pyspark.sql.window import Window
...
# DBTITLE 1,Is the API pipeline healthy? Are all cities returning data on schedule
from pyspark.sql import functions as F
